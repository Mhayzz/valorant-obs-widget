...
// ── Demo endpoints (preview publique, pas de config requise) ─
const demoCache = new Map();

function getDemoCache(key) {
  const entry = demoCache.get(key);
  if (!entry) return null;
  if (Date.now() - entry.ts > 60000) { demoCache.delete(key); return null; }
  return entry.data;
}

function setDemoCache(key, data) {
  if (demoCache.size > 50) {
    const oldest = [...demoCache.entries()].sort((a, b) => a[1].ts - b[1].ts)[0];
    if (oldest) demoCache.delete(oldest[0]);
  }
  demoCache.set(key, { data, ts: Date.now() });
}

app.get("/api/demo/rank", async (req, res) => {
  const name   = (req.query.name   || "").trim();
  const tag    = (req.query.tag    || "").trim();
  const region = (req.query.region || "eu").trim();
  if (!name || !tag) return res.status(400).json({ error: "name et tag requis" });

  const cacheKey = `rank/${region}/${name}#${tag}`.toLowerCase();
  const cached = getDemoCache(cacheKey);
  if (cached) return res.json(cached);

  const { henrik_api_key: apiKey } = getCfg();
  const headers = { "Content-Type": "application/json" };
  if (apiKey) headers["Authorization"] = apiKey;
  ...
  try {
    ...
    setDemoCache(cacheKey, result);
    res.json(result);
  } catch(e) { res.status(500).json({ error: "Erreur serveur" }); }
});

app.get("/api/demo/matches", async (req, res) => {
  const name   = (req.query.name   || "").trim();
  const tag    = (req.query.tag    || "").trim();
  const region = (req.query.region || "eu").trim();
  const size   = Math.min(5, Math.max(1, parseInt(req.query.size) || 5));
  if (!name || !tag) return res.status(400).json({ error: "name et tag requis" });
  ...
  try {
    ...
    if (r.ok && Array.isArray(json.data) && json.data.length > 0) {
      const matches = json.data.map(m => {
        const stats = m.stats; const meta = m.meta; const teams = m.teams;
        if (!stats) return null;
        const agentId   = stats.character?.id;
        const agentName = stats.character?.name || "Unknown";
        const agentIcon = agentId ? `https://media.valorant-api.com/agents/${agentId}/displayicon.png` : null;
        const playerTeam = (stats.team || "").toLowerCase();
        const redRounds  = teams?.red  ?? 0;
        const blueRounds = teams?.blue ?? 0;
        const won = playerTeam === "red"  ? redRounds  > blueRounds
                  : playerTeam === "blue" ? blueRounds > redRounds : null;
        const mapRaw = meta?.map;
        const mapName = typeof mapRaw === "string" ? mapRaw : (mapRaw?.name || "Unknown");
        return { agent: agentName, agent_icon: agentIcon, kills: stats.kills ?? 0, deaths: stats.deaths ?? 0, assists: stats.assists ?? 0, won, map: mapName, mode: meta?.mode || "" };
      }).filter(Boolean);
      setDemoCache(cacheKey, matches);
      return res.json(matches);
    }
  } catch(e) { console.error("demo v1/lifetime:", e.message); }
  ...
  try {
    ...
    const matches = (json.data || []).map(match => {
      ...
      const queueRaw = match.metadata?.queue;
      const mode     = typeof queueRaw === "string" ? queueRaw : (queueRaw?.name || queueRaw?.id || "");
      return { agent: player.agent?.name || "Unknown", agent_icon: agentIcon, kills: player.stats?.kills ?? 0, deaths: player.stats?.deaths ?? 0, assists: player.stats?.assists ?? 0, won: team?.won ?? team?.has_won ?? null, map: mapName, mode };
    }).filter(Boolean);

    setDemoCache(cacheKey, matches);
    res.json(matches);
  } catch(e) {
    console.error("demo v3/matches:", e.message);
    res.status(500).json({ error: "Erreur serveur" });
  }
});
